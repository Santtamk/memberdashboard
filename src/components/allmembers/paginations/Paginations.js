...
const Pagination = ({ handlePageChange, totalPages, currentPage, itemsPerPage, employees }) => {
  return (
    <div>
          ...
          <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">{Math.min(currentPage * itemsPerPage, employees.length)}</span> of{" "}
                ...
                {[...Array(totalPages)].map((_, index) => (
                  <button
                    ...
                    className={`relative z-10 inline-flex items-center px-4 py-2 text-sm font-semibold ${
                      currentPage === index + 1
                        ? "bg-sky-200 text-sky-400 rounded-lg"
